...
public class LoanRepayment {
  ...
  public static void SetSchemaNames(String coreDBSchema, String channelSchema) {
    LoanRepayment.coreDBSchema = coreDBSchema;
    LoanRepayment.channelSchema = channelSchema;
  }
  ...
  @APIResponseSchema(
      value = LoanRepaymentObject.class,
      responseDescription = "Loan Repayment Response",
      responseCode = "200")
  @Operation(summary = "Loan Repayment Request", description = "returns Loan Repayment data")
  public Response getLoanRepaymentDetails(
      @RequestBody(
    ...
    try {
      System.out.println("Loan Repayment Posting Started on [" + startTime + "]");
      TransId = id.getHdrTranId();
      SourceUniqRef = id.getReqTransactionReferenceNo();
      String DebitAccountno = id.getReqAccountNo();
      String CreditAccountno = id.getReqBeneficiaryAccount();
      String DebitAccountCurrency = id.getReqDebitAcCurrency();
      String DebitAmount = id.getReqDebitAmount();
      String UnitId = id.getReqUnitID();
      String channelId = id.getReqChnlId();
      String ServiceName = "LoanRepayment";
      String CBXReference = "";
      ...
      System.out.println("Fetching Loan Repayment Details Transaction Id [" + SourceUniqRef + "]");
      ...
      try {
        if (ValidationStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())
          ...
          Field[] fields = LoanRepaymentRequest.class.getDeclaredFields();
      ...
      try {
        if (QueueStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())) {
          ...
          Field[] resfields = LoanRepaymentObject.class.getDeclaredFields();
          for (Field field : resfields) {
            if (field.isAnnotationPresent(JsonbProperty.class)) {
              String annotationValue = field.getAnnotation(JsonbProperty.class).value();
              if (field.get(IntTransObj) != null)
                RespDetailMap.put(
                    annotationValue, field.get(IntTransObj).toString().replaceAll("\"", "\\\""));
            }
          }
      ...
      System.out.println(
          "Loan Repayment Posting Completed and Processing Time Taken [ "
              + millis
              + " ] MilliSeconds");
    }
  }
  ...
  private void ResponseMessages(
      LoanRepaymentObject IntTransObj,
      String SourceUniqRef,
      String UniqueReference,
    ...
    IntTransObj.setResResultCode(ErrorCode);
    IntTransObj.setResCbxReferenceNo(SourceUniqRef);
    IntTransObj.setResUserId(UnitId);

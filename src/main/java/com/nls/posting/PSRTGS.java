...
public class PSRTGS {
  ...
  @APIResponseSchema(
      value = PSRTGSObject.class,
      responseDescription = "RTGS Response",
      responseCode = "200")
  @Operation(summary = "RTGS Request", description = "returns RTGS data")
  public Response getRTGSDetails(
      @RequestBody(
              ...
              content =
                  @Content(
                      ...
                      schema = @Schema(implementation = PSRTGSRequest.class)))
          PSRTGSRequest id) {
    ...
    try {
      System.out.println("RTGS Transfer Posting Started on [" + startTime + "]");
      TransId = id.transactionId;
      SourceUniqRef = id.transReference;
      String DebitAccountno = id.accountNo;
      // String CreditAccountno = id.benefAccount;
      String CreditAccountno = "";
      String DebitAccountCurrency = id.debitAccCurrency.toUpperCase();
      String CreditAccountCurrency = id.creditAccCurrency.toUpperCase();
      String DebitAmount = id.debitAmt;
      String UnitId = id.unitId;
      String channelId = id.channelId;
      String ServiceName = "RTGS";
      String CBXReference = "";
      String BICCode = id.benefBankBIC;
      ...
      System.out.println("Fetching RTGS Details Transaction Id [" + SourceUniqRef + "]");
      ...
      try {
        if (ValidationStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())
          ...
          Field[] fields = PSRTGSRequest.class.getDeclaredFields();
          ...
          System.out.println("OFSData[" + OFSData + "]");
          CreditAccountno = GlobalParameters.get(ServiceName + "_" + CreditAccountCurrency);
      ...
      try {
        if (QueueStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())) {
          ...
          Field[] resfields = PSRTGSObject.class.getDeclaredFields();
          for (Field field : resfields) {
            if (field.isAnnotationPresent(JsonbProperty.class)) {
              String annotationValue = field.getAnnotation(JsonbProperty.class).value();
              if (field.get(RTGSObj) != null)
                RespDetailMap.put(
                    annotationValue, field.get(RTGSObj).toString().replaceAll("\"", "\\\""));
            }
          }
      ...
      System.out.println(
          "RTGS Transfer Posting Completed and Processing Time Taken [ "
              + millis
              + " ] MilliSeconds");
    }
  }
  ...
  private void ResponseMessages(
    ...
    RTGSObj.setResultCode(ErrorCode);
    RTGSObj.setCbxReferenceNo(SourceUniqRef);
    RTGSObj.setUserId(UnitId);
    RTGSObj.setStatusCode(ErrorCode);

...
public class ChargeTransaction {
  ...
  @APIResponseSchema(
      value = ChargeTransactionObject.class,
      responseDescription = "ChargeTransaction Response",
      responseCode = "200")
  @Operation(summary = "ChargeTransaction Request", description = "returns ChargeTransaction data")
  public Response getChargeTransactionDetails(
      @RequestBody(
              ...
              content =
                  @Content(
                      ...
                      schema = @Schema(implementation = ChargeTransactionRequest.class)))
          ChargeTransactionRequest id) {

    ChargeTransactionObject CCRObj = new ChargeTransactionObject();
    ...
    try {
      System.out.println("Charge Transaction Posting Started on [" + startTime + "]");
      TransId = id.hdrTranId;
      SourceUniqRef = id.reqTransactionReferenceNo;
      String DebitAccountno = id.reqAccountNo;
      String CreditAccountNumber = id.reqCreditAccountNo;
      String CreditAccountno = id.reqCreditAccountNo.startsWith("PL") ? "" : id.reqCreditAccountNo;
      ...
      String ServiceName = "ChargeTransaction";
      String CBXReference = "";
      VIPMarker = id.vipMarker.equalsIgnoreCase(ResponseStatus.Y.getValue()) ? "VIP" : "";
      // WaiveCharge = id.waiveCharge.equalsIgnoreCase(ResponseStatus.N.getValue()) ? "false" :
      // "true";
      WaiveCharge = "false";

      System.out.println(
          "Fetching Charge Transaction Details For Transaction Ref [" + SourceUniqRef + "]");
      ...
      try {
        if (ValidationStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())
          ...
          Field[] fields = ChargeTransactionRequest.class.getDeclaredFields();
      ...
      try {
        if (QueueStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())) {
          ...
          Field[] resfields = ChargeTransactionObject.class.getDeclaredFields();
          for (Field field : resfields) {
            if (field.isAnnotationPresent(JsonbProperty.class)) {
              String annotationValue = field.getAnnotation(JsonbProperty.class).value();
      ...
      System.out.println(
          "Charge Transaction Posting Completed and Processing Time Taken [ "
              + millis
              + " ] MilliSeconds");
    }
  }
  ...
  private void ResponseMessages(
      ChargeTransactionObject CCRObj,
      String SourceUniqRef,
      String UniqueReference,

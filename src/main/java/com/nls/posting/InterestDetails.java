...
public class InterestDetails {
  ...
  @APIResponseSchema(
      value = InterestObject.class,
      responseDescription = "Interest Response",
      responseCode = "200")
  @Operation(summary = "Interest Request", description = "returns Interest data")
  public Response getInterestDetails(
      @RequestBody(
              ...
              content =
                  @Content(
                      ...
                      schema = @Schema(implementation = InteresRequest.class)))
          InteresRequest id) {

    InterestObject interestObj = new InterestObject();
    ...
    try {
      System.out.println("Interest Posting Started on [" + startTime + "]");
      ...
      String ServiceName = "Interest";
      String CBXReference = "";
      String DebitAccountno = GlobalParameters.get("INTEREST_DEBIT_ACCOUNT");
      ...
      System.out.println(
          "Fetching Internal Transfer Details Transaction Id [" + SourceUniqRef + "]");
      ...
      try {
        if (ValidationStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())
          ...
          Field[] fields = InteresRequest.class.getDeclaredFields();
      ...
      try {
        if (QueueStatus.equalsIgnoreCase(ResponseStatus.SUCCESS.getValue())) {
          ...
          Field[] resfields = InterestObject.class.getDeclaredFields();
          for (Field field : resfields) {
            if (field.isAnnotationPresent(JsonbProperty.class)) {
              String annotationValue = field.getAnnotation(JsonbProperty.class).value();
              if (field.get(interestObj) != null)
                RespDetailMap.put(
                    annotationValue, field.get(interestObj).toString().replaceAll("\"", "\\\""));
            }
          }
      ...
      System.out.println(
          "Interest Posting Completed and Processing Time Taken [ " + millis + " ] MilliSeconds");
    }
  }

  private void ResponseMessages(
      InterestObject interestObj,
      String SourceUniqRef,
      String UniqueReference,
    ...
    interestObj.setResCbxReferenceNo(SourceUniqRef);
    interestObj.setResUserId(UnitId);
    interestObj.setResStatusCode(ErrorCode);
